...
Revelado* revelado_newParametros(char* idStr,char* fechaStr,char* tipoStr, char* cantidadStr, char* precioStr, char* cuitStr)
{
    ...
    if(pRevelado != NULL && idStr != NULL && fechaStr != NULL && tipoStr != NULL && cantidadStr != NULL && precioStr != NULL && cuitStr != NULL)
    {
        /*if(!revelado_setIdStr(pRevelado, idStr) &&
                ...
                !revelado_setCantidadStr(pRevelado, cuitStr))
        {*/
...
/** \brief Valida el elemento tipo del revelado y lo carga en la estructura Revelado.
 *
 * \param this Revelado* puntero al revelado.
 * \param tipo char* tipo del revelado.
...
/** \brief Obtiene el tipo de un revelado.
 *
 * \param this Revelado* puntero a revelado.
 * \param resultado char* tipo de revelado
...
/** \brief Valida el elemento cantidad del revelado y lo carga en la estructura Revelado.
 *
 * \param this Revelado* puntero al revelado.
 * \param cantidad char* cantidad del revelado.
...
/** \brief Obtiene el cantidad de un revelado.
 *
 * \param this Revelado* puntero a revelado.
 * \param resultado char* cantidad de revelado
...
/** \brief Valida el elemento importe de la revelado y lo carga en la estructura Revelado
 *
 * \param this Revelado* puntero al revelado
 * \param importe int importe del revelado.
...
/** \brief Obtiene el importe de un revelado.
 *
 * \param this Revelado* puntero a revelado.
 * \param resultado int* importe de Revelado
...
/** \brief Valida el elemento id string de la revelado y lo carga en la estructura Revelado.
 *
 * \param this Revelado* puntero al revelado.
 * \param id char* id de la revelado.
 * \return int retorno -1 si no pudo validar el id.
 *                      0 si pudo validar y cargar el id.
 *
 */
...
/** \brief Obtiene el id de tipo string de una revelado.
 *
 * \param this Revelado* puntero a revelado.
 * \param resultado char* ID de revelado
 * \return int retorno -1 si this es NULL o ID es NULL,
 *                      0 si se obtuvo el ID.
 *
 */
...
/** \brief Valida el elemento cantidad de la revelado y lo carga en la estructura Revelado.
 *
 * \param this Revelado* puntero al revelado.
 * \param cantidad char* cantidad del revelado.
 * \return int retorno -1 si no pudo validar el cantidad.
...
/** \brief Obtiene el cantidad de tipo string de un revelado.
 *
 * \param this Revelado* puntero a revelado.
 * \param resultado char* cantidad de revelado
 * \return int retorno -1 si this es NULL o cantidad es NULL,
...
/** \brief Valida el elemento importe del revelado y lo carga en la estructura Revelado.
 *
 * \param this Revelado* puntero al revelado.
 * \param importe char* importe del revelado.
 * \return int retorno -1 si no pudo validar el importe.
 *                      0 si pudo validar y cargar el importe.
 *
 */
...
int revelado_ventasMAyor150 (void* this)
{
    ...
    if(this!= NULL)
    {
        revelado_getPrecio((Revelado*)this, &auxRevelado);
        if(auxRevelado > 150)
        {
            ret=1;
        }
    ...
    if(pArrayListRevelado != NULL)
    {
        for(i=0; i<=ll_len(pArrayListRevelado); i++)
        {
            ...
            if(pAuxRevelado != NULL)
            {
                ...
                {
                    sumadorFotos += auxFotos;
                    ret = 0;
                }
            }
        }
        *fotosTotales = sumadorFotos;
    }
    return ret;
}
...
int revelado_polaroid (void* this)
{
    ...
    if(this!= NULL)
    {
        revelado_getTipo((Revelado*)this, auxEntrega);
        if(!strcmp("POLAROID", auxEntrega))
        {
            ret=1;
        }
...
/*int revelado_importeTotal (LinkedList* pArrayListRevelado, float* importeRevelados)
{
    int ret = -1;
    int i = 0;
    Revelado* pAuxRevelado;
    ...
    if(pArrayListRevelado != NULL)
    {
        for(i=0; i<=ll_len(pArrayListRevelado); i++)
        {
            ...
            if(pAuxRevelado != NULL)
            {
                if(!revelado_getImporte(pAuxRevelado, &auxImporte))
                {
        ...
        *importeTotalRevelados = sumadorImporte;
    }
    return ret;
}*/
